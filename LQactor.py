...
delta_tau = 0.5
milestones = [100,200]
decay = 1 # increase the learning rate due to flatness
num_trig_basis = 3
...
def V_pt(t,x): # true value function in numpy
    # x is N x d; V is N x 1
    temp = torch.sum(beta_pt * torch.sin(x), dim=-1, keepdim=True)
    return temp + beta0 * (T-t)
...
#     return temp / 2 + beta0 + np.sum(u**2, axis=-1, keepdims=True)
...
x0_valid = np.random.uniform(0,2*np.pi,[N_valid,d])
...
def train(Control_NN,actor_optimizer,actor_scheduler):
    ...
    for step in range(num_steps+1):
        ...
        for actor_step in range(num_actor_update):
            ...
            if step>=200 and previous_loss < loss.item():
                break
